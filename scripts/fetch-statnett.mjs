...
async function readGridRows(grid){
...
function parseGrid(rows,status){
  ...
  return rows.slice(1).filter(r=>r[iArea] && /^NO\d$/i.test(r[iArea])).map(r=>({
...
async function extractReport({heading,listText,status,key}){
  ...
  const diagnostics=[];
  for(let i=0;i<gc;i++){
    const rows=await readGridRows(grids.nth(i));
    const p=parseGrid(rows,status);
    const productionCount=p.filter(x=>productionTypes.has(x.industry)).length;
    diagnostics.push({grid:i,row_count:rows.length,parsed_count:p.length,production_count:productionCount,sample:rows.slice(0,4)});
    parsed.push({rows:p,productionCount});
  }
  // Velg forbrukstabellen: flest ikke-produksjonsrader. Dette hindrer at vann/sol/vind blandes inn i forbrukskøen.
